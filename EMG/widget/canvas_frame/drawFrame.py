...
class drawFrame(MyCanvas):
    ...
    def __init__(self, parent=None):
        super(drawFrame, self).__init__(parent)
        self.initUI()
        self.initValues()
    ...
    def initUI(self):
        ...
        self.btn_identify.clicked.connect(self.btn_identify_clicked)    # 测试按钮
    ...
    def btn_identify_clicked(self): # 测试按钮点击事件
        if self.test_flag:  # 如果测试标志为True --> 关闭测试
            self.test_flag = False
            self.btn_identify.setText('识别')
        else:
            self.test_flag = True
            self.btn_identify.setText('停止')
